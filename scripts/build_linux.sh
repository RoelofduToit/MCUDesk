...
OUTPUT_DIRECTORY="${PROJECT_ROOT}/dist/MCUDesk"

if [[ ! -x "${PYTHON_EXECUTABLE}" ]]; then
    echo "MCUDesk project environment not found: ${PYTHON_EXECUTABLE}" >&2
...
rm -rf -- "${PROJECT_ROOT}/build/MCUDesk" "${OUTPUT_DIRECTORY}"

"${PYTHON_EXECUTABLE}" -m PyInstaller \
    --noconfirm \
    --clean \
    --workpath "${PROJECT_ROOT}/build/MCUDesk" \
    --distpath "${PROJECT_ROOT}/dist" \
    "${SPEC_FILE}"

echo "MCUDesk Linux bundle created at: ${OUTPUT_DIRECTORY}"
echo "Launch it with: ${OUTPUT_DIRECTORY}/MCUDesk"
