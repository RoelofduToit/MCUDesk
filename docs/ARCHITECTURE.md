...
Dashboard sparkline history remains centralized and source-keyed rather than living in each tile. It retains up to one hour of actual timestamped measurements with a separate emergency point ceiling. The global `appearance/dashboard_trend_window_seconds` preference selects only the visible 30-second to one-hour interval; changing it redraws existing tiles from retained history. Each tiny sparkline is visually thinned to at most 48 values, while Graphs, recording, session files, and exports continue using their unchanged authoritative measurements. Replay loads the same history from recorded elapsed timestamps.
